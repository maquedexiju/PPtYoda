...
import browsercookie # pip install browser-cookie3
...
def copy_cookie_file():
    ...
    cookies_path = list(chrome.find_cookie_files())[0]
    unlock_cookies(cookies_path)
    dest_path = os.path.join('chrome-data', cookies_path.split('User Data\\')[1])
    unlock_cookies(dest_path)

    shutil.copy(cookies_path, dest_path)